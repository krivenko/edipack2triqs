...
class TestEDIpackSolverPhononsNoBath(TestSolver):
    ...
    def test_zerotemp(self):
        ...
        ## Reference solution
        refs = self.ref_results("zerotemp", h=h, phonon=phonon, fops=fops,
    ...
    def test_nspin1(self):
        ...
        ## Reference solution
        refs = self.ref_results("nspin1_1", h=h, phonon=phonon,
                                fops=fops, **solve_params)
        self.assert_all(solver, **refs)

        # Part II: update phonon coupling
        g_ph = np.diag([0.2, 0.3])
        a_ph = 0.06
        phonon = PhononParams(
            frequency=0.5,
            coupling=self.make_h_loc(mul.outer(s0, g_ph)) + a_ph,
        ...
        ## Reference solution
        refs = self.ref_results("nspin1_2", h=h, phonon=phonon,
                                fops=fops, **solve_params)
        self.assert_all(solver, **refs)
    ...
    def test_nspin2(self):
        ...
        ## Reference solution
        refs = self.ref_results("nspin2_1", h=h, phonon=phonon,
                                fops=fops, **solve_params)
        self.assert_all(solver, **refs)
        ...
        ## Reference solution
        refs = self.ref_results("nspin2_2", h=h, phonon=phonon,
                                fops=fops, **solve_params)
        self.assert_all(solver, **refs)
    ...
    def test_nonsu2(self):
        ...
        ## Reference solution
        refs = self.ref_results("nonsu2_hloc_1", h=h, phonon=phonon, fops=fops,
                                spin_blocks=False, **solve_params)
        self.assert_all(solver, **refs)

        # Part II: update interaction parameters
        g_ph = np.array([[0.2, 0.2],
                         [0.2, 0.3]])
        a_ph = 0.06
        phonon_coupling = self.make_h_loc(mul.outer(s0, g_ph),
                                          spin_blocks=False) + a_ph
        ...
        ## Reference solution
        refs = self.ref_results("nonsu2_hloc_2", h=h, phonon=phonon, fops=fops,
                                spin_blocks=False, **solve_params)
        self.assert_all(solver, **refs)
    ...
    def test_h_loc_an(self):
        ...
        # Reference solution
        refs = self.ref_results("h_loc_an_1", h=h, phonon=phonon,
        ...
        # Reference solution
        refs = self.ref_results("h_loc_an_2", h=h, phonon=phonon,
                                fops=fops, superc=True, **solve_params)
        self.assert_all(solver, **refs)


# TODO: Switch to hybrid bath with nbath = 3?
class TestEDIpackSolverPhononsNormal(TestSolver):
